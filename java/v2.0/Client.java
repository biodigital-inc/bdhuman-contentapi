...
import com.google.gson.JsonElement;
 ...
 class Globals {
	...
	public static String OAUTH_TOKEN_URL = "https://apis.biodigital.com/oauth2/v2/token/";

	// BioDigital Content API endpoint for collection requests
	public static String CONTENTAPI_COLLECTIONS_URL = "https://apis.biodigital.com/services/v2/content/collections/";
...
public class Client {
		...
		if (auth_str != ""){
			...
			if ((response_code == HttpURLConnection.HTTP_BAD_REQUEST) || (response_code == HttpURLConnection.HTTP_INTERNAL_ERROR)){
				...
				if (access_token.equals("")){
					...
					System.out.println("   2) Request a new access token");
					System.out.println("   3) Return a list of all my dashboard content:  /myhuman");
					System.out.println("   4) Return a list of all my team collections: /mycollections");
					System.out.println("");
					int option = s.nextInt(); 

					if (option == 2){
						System.out.println("Making request, please wait ...");
						...
						System.out.println("Making request, please wait ...");
						query_collection_mycollections(access_token);
					}
				} 
	...
	private static void query_collection_mycollections(String access_token) throws IOException {
	...
	// ***********************************************************
		...
		HashMap<String, String> params = new HashMap<String, String>();
		params.clear();
		String url = Globals.CONTENTAPI_COLLECTIONS_URL + "mycollections";
		HttpURLConnection connection = build_get_request(url,params,access_token);
		String[] response_parts = build_response(connection).split(":",2);
		int response_code = Integer.parseInt(response_parts[0]);
		String response_body = response_parts[1];
		...
		if ((response_code == HttpURLConnection.HTTP_BAD_REQUEST) || (response_code == HttpURLConnection.HTTP_INTERNAL_ERROR)){
			...
			System.out.println("Select a number option below:");
			System.out.println("   1) Exit");
			System.out.println("   2) Request a new access token");
			System.out.println("   3) Return a list of all my dashboard content:  /myhuman");
			System.out.println("   4) Return a list of all my team collections: /mycollections");

			JsonObject jsonObject = new JsonParser().parse(response_body).getAsJsonObject();
			final JsonArray collections = jsonObject.getAsJsonArray("mycollections");
			int index = 5;
			String option_value = "";
			JsonObject mycollections_id_obj = new JsonObject();
			JsonObject mycollections_id_contentlist_obj = new JsonObject();
			List<String> list = new ArrayList<String>();
			for (JsonElement element : collections) {
				String collection_id = ((JsonObject) element).get("collection_id").getAsString();
				String collection_name = ((JsonObject) element).get("collection_name").getAsString();
				System.out.println("");
				System.out.println("   Team Collection:  "+ collection_name + " [id: " + collection_id + "]" );

				option_value = Integer.toString(index);
				System.out.println("   " + option_value + ") Return details for this team collection only:  /mycollections/"+collection_id);
				mycollections_id_obj.addProperty(option_value, collection_id);
				index += 1;

				option_value = Integer.toString(index);
				System.out.println("   " + option_value + ") Return a list of all content for this team collection:  /mycollections/"+collection_id+"/content_list");
				mycollections_id_contentlist_obj.addProperty(option_value, collection_id);
				index += 1;
			}
			System.out.println("");
			System.out.println("Type your number selection, then hit enter:  ");
			int option = s.nextInt(); 
			...
			else if (mycollections_id_obj.has(Integer.toString(option))) 
			{
				query_collection_mycollections_id(access_token, mycollections_id_obj.get(Integer.toString(option)).getAsString());
			}
			else if (mycollections_id_contentlist_obj.has(Integer.toString(option))) {
				query_collection_mycollections_id_content_list(access_token, mycollections_id_contentlist_obj.get(Integer.toString(option)).getAsString());
			}
	...
	private static void query_collection_mycollections_id(String access_token, String id) throws IOException {
	...
	// ***********************************************************
		...
		HashMap<String, String> params = new HashMap<String, String>();
		params.clear();
		String url = Globals.CONTENTAPI_COLLECTIONS_URL + "mycollections/"+id;
		HttpURLConnection connection = build_get_request(url,params,access_token);
		String[] response_parts = build_response(connection).split(":",2);
		int response_code = Integer.parseInt(response_parts[0]);
		String response_body = response_parts[1];
	...
	private static void query_collection_mycollections_id_content_list(String access_token, String id) throws IOException {
	...
	// ***********************************************************
		...
		HashMap<String, String> params = new HashMap<String, String>();
		params.clear();
		String url = Globals.CONTENTAPI_COLLECTIONS_URL + "mycollections/"+id+"/content_list";
		HttpURLConnection connection = build_get_request(url,params,access_token);
		String[] response_parts = build_response(connection).split(":",2);
		int response_code = Integer.parseInt(response_parts[0]);
		String response_body = response_parts[1];
		...
		if ((response_code == HttpURLConnection.HTTP_BAD_REQUEST) || (response_code == HttpURLConnection.HTTP_INTERNAL_ERROR)){
			...
			System.out.println("   3) Return a list of all my dashboard content:  /myhuman");
			System.out.println("   4) Return a list of all my team collections: /mycollections");
			System.out.println("   5) Return details for this team collection only:  /mycollections/"+id);
			...
			if (option == 2){
				...
				query_collection_mycollections_id(access_token, id);
			}
